...
function _buildAdminMbaasParams(params){
  var basePath = config.addURIParams(constants.ADMIN_API_PATH, params);
  params = params || {};
...
function doFHMbaaSRequest(params, cb){
  ...
  if(params.fileRequest && params.fileUploadRequest){
    ...
    //Registering Stream Event Listeners
    fileData.stream.on('end', function(){
      cb();
    });
    ...
    return fileData.stream.pipe(request(params));
...
function _buildAppRequestParams(params){
  ...
  //Setting headers
  if(params.fileRequest && params.fileUploadRequest){
    headers['content-type'] = params.data.type;
    headers['content-length'] = params.data.size;
  }
...
function adminRequest(params, cb){
  params = params || {};
  var fullParams = _.extend(_.clone(params), config.getEnvironmentConfig(params));
