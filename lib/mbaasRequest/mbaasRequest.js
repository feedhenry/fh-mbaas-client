...
/**
 * Build request params for a storage request to an Mbaas
 * @param params
 * @returns Object
 * @private
 */
function _buildStorageRequestParams(params) {
  var mbaasConf = params[constants.MBAAS_CONF_KEY];
  var mbaasUrl = url.parse(mbaasConf.__mbaasUrl);
  mbaasUrl.pathname = params.resourcePath;
  ...
  return {
    ...
    auth: {
      user: mbaasConf.username,
      pass: mbaasConf.password
    }
  };
}
...
/**
 * Perform a request against the storage MBaaS API
 * @param params
 * @param cb
 */
function storageRequest(params, cb) {
  params = params || {};
  log.logger.debug({params: params}, "FH-MBAAS-CLIENT: storageRequest ");

  var fullParams = _buildStorageRequestParams(params);
  log.logger.debug({fullParams: fullParams}, "FH-MBAAS-CLIENT: storageRequest ");
